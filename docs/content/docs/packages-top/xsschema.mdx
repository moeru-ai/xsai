...
Pre-bundled version of `StandardJSONSchemaV1` in `@standard-schema/spec`. Use this when you want a schema that can also provide JSON Schema directly, for example when building tool definitions without an extra conversion step.
