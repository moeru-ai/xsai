...
- `format` is the structured-output request schema (raw JSON Schema or `StandardJSONSchemaV1`), always sent strict; `resolveFormat` produces the wire-ready form once per call and each adapter maps it to its format field (`response_format` / `text.format` / `output_config.format`).
