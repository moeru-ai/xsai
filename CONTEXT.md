...
**Terminal event**:
A wire that delivers a terminal signal ends with exactly one `stream.end` event, emitted when the stream closes; an `error` stream end carries the terminating error on its `error` field. A wire stream that ends without a terminal signal is truncated — the stream rejects instead of ending.
_Avoid_: end-of-stream sentinel

**Collect**:
The consumer-side helper `collect(model, options)` that resolves the `stream.end` payload and rejects on `reason: 'error'` or a stream rejection. In-progress rendering consumes the `Event` stream directly.
...
- `stream.end` separates two identities: `message.id` is the replayable assistant message id; `responseId` is the response-scoped generation id (e.g. `chatcmpl-*`, `resp_*`). A wire may carry both, either, or neither.
