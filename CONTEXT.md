...
- `format` is the structured-output request schema (raw JSON Schema or `StandardJSONSchemaV1`), always sent strict; `resolveSchema` (shared with `tool()`) resolves it once per call and each adapter applies strict/name policy and maps it to its format field (`response_format` / `text.format` / `output_config.format`).
